// ============================================================
//  Diego's Adventure — game.js  v1.8
//  9/16 = 360x640 | WORLD_H=2560 | plateformes ~120px écart
// ============================================================
...
// Plateformes espacées de 110-130px — sautables avec JUMP=-13
// Alternance gauche/droite, étoiles placées entre plateformes
const LEVELS = [
  {
    ...
    platforms:[
      {x:0,   y:2480,w:360,h:16}, // sol
      {x:20,  y:2360,w:160,h:16}, {x:190,y:2240,w:160,h:16},
      {x:20,  y:2120,w:160,h:16}, {x:190,y:2000,w:160,h:16},
      {x:20,  y:1880,w:160,h:16}, {x:190,y:1760,w:160,h:16},
      {x:20,  y:1640,w:160,h:16}, {x:190,y:1520,w:160,h:16},
      {x:20,  y:1400,w:160,h:16}, {x:190,y:1280,w:160,h:16},
      {x:20,  y:1160,w:160,h:16}, {x:190,y:1040,w:160,h:16},
      {x:20,  y: 920,w:160,h:16}, {x:190,y: 800,w:160,h:16},
      {x:20,  y: 680,w:160,h:16}, {x:190,y: 560,w:160,h:16},
      {x:20,  y: 440,w:160,h:16}, {x:190,y: 320,w:160,h:16},
      {x:80,  y: 200,w:200,h:16}, {x:80,  y:  80,w:200,h:16},
    ],
    stars:[
      {x:100,y:2420}, {x:260,y:2060},
      {x:100,y:1700}, {x:260,y:1220},
      {x:180,y:40},
    ],
  },
  {
    ...
    platforms:[
      {x:0,   y:2480,w:360,h:16},
      {x:190,y:2360,w:160,h:16}, {x:20, y:2240,w:160,h:16},
      {x:190,y:2120,w:160,h:16}, {x:20, y:2000,w:160,h:16},
      {x:190,y:1880,w:160,h:16}, {x:20, y:1760,w:160,h:16},
      {x:190,y:1640,w:160,h:16}, {x:20, y:1520,w:160,h:16},
      {x:190,y:1400,w:160,h:16}, {x:20, y:1280,w:160,h:16},
      {x:190,y:1160,w:160,h:16}, {x:20, y:1040,w:160,h:16},
      {x:190,y: 920,w:160,h:16}, {x:20, y: 800,w:160,h:16},
      {x:190,y: 680,w:160,h:16}, {x:20, y: 560,w:160,h:16},
      {x:190,y: 440,w:160,h:16}, {x:20, y: 320,w:160,h:16},
      {x:80, y: 200,w:200,h:16}, {x:80, y:  80,w:200,h:16},
    ],
    stars:[
      {x:260,y:2420}, {x:100,y:2060},
      {x:260,y:1700}, {x:100,y:1220},
      {x:180,y:40},
    ],
  },
  {
...
const GROUND_Y = 2480;
const player = {x:60,y:2430,w:28,h:32,vx:0,vy:0,onGround:false,facing:1,frame:0,ft:0};
...
function startGame() {
  ...
  // iOS : initAudio DANS le même geste que le bouton play
  initAudio();
  gameState='playing';
  initLevel();
  createMuteButton();
...
function startTimer() {
  ...
  timerInterval=setInterval(()=>{
    ...
    if (--timeLeft<=0) {
      timeLeft=0;
      clearInterval(timerInterval);
      player.x=60; player.y=2430; player.vx=0; player.vy=0;
      cameraY=WORLD_H-H;
...
function spawnCoin() {
  coins.push({
    x:20+Math.random()*(W-40), y:cameraY-20,
    vy:0, vx:(Math.random()-0.5)*1.5,
    onPlat:false, life:600, pulse:Math.random()*Math.PI*2,
    collected:false,
  });
}
function updateCoins() {
  if(++coinSpawnTimer>=60){ coinSpawnTimer=0; spawnCoin(); }
  ...
  for (const c of coins) {
    ...
    if (c.onPlat) {
      ...
      for (const p of lvl.platforms)
        if(c.x>p.x&&c.x<p.x+p.w&&Math.abs(c.y-p.y)<6){still=true;break;}
      if(!still) c.onPlat=false;
    } else {
      c.vy+=GRAVITY*0.7; c.y+=c.vy; c.x+=c.vx;
      for (const p of lvl.platforms) {
        if(c.x>p.x&&c.x<p.x+p.w&&c.y>p.y-4&&c.y<p.y+12&&c.vy>0){
          c.y=p.y; c.vy*=-0.2; c.onPlat=true; break;
        }
      }
      if(c.y>=GROUND_Y){c.y=GROUND_Y;c.vy*=-0.15;c.onPlat=true;}
    }
    c.pulse+=0.08; c.life--;
    if(c.x<-20||c.x>W+20) c.life=0;
    const dx=player.x+player.w/2-c.x, dy=player.y+player.h/2-c.y;
    if(Math.sqrt(dx*dx+dy*dy)<22){
      c.collected=true; c.life=0; coinsCollected++;
      spawnParticles(c.x,c.y-cameraY,'#ffd700'); updateUI();
    }
  }
...
function initLevel() {
  ...
  coins=[]; coinsCollected=0; coinSpawnTimer=0;
  cameraY=WORLD_H-H;
...
function update() {
  ...
  let speed=0;
  if(padActive){const r=padRatio(padX);if(Math.abs(r)>0.04){speed=r*PAD_MAX_SPEED;player.facing=r>0?1:-1;}}
  ...
  if((keys['Space']||keys['ArrowUp'])&&player.onGround){player.vy=JUMP;player.onGround=false;}
  ...
  if(player.y>WORLD_H+100){player.x=60;player.y=2430;player.vy=0;}
  ...
  for(const s of stars){
    ...
    if(Math.sqrt(dx*dx+dy*dy)<26){
      ...
      spawnParticles(s.x,s.y-cameraY,'#ffd700');updateUI();
...
function drawGround(){
  const lvl=LEVELS[currentLevel];
  ctx.fillStyle=lvl.groundColor; ctx.fillRect(0,GROUND_Y+16,W,200);
...
function drawCoins(){
  for(const c of coins){
    if(c.collected||c.y<cameraY-20||c.y>cameraY+H+20) continue;
    const pulse=1+Math.sin(c.pulse)*.1, r=8*pulse;
    ctx.save(); ctx.globalAlpha=0.3; ctx.fillStyle='#ffd700';
    ctx.beginPath(); ctx.arc(c.x,c.y,r+4,0,Math.PI*2); ctx.fill(); ctx.restore();
...
function drawStars(){
  for(const s of stars){
    if(s.collected||s.y<cameraY-40||s.y>cameraY+H+40) continue;
    const p=1+Math.sin(s.pulse)*.12,r=16*p;
    ctx.save(); ctx.globalAlpha=.25+Math.sin(s.pulse)*.1;
    ctx.fillStyle='#ffd700'; ctx.beginPath(); ctx.arc(s.x,s.y,r+7,0,Math.PI*2); ctx.fill(); ctx.restore();
...
function drawEarth(){
  [45,170,300].forEach(x=>{
    ctx.fillStyle='#4a2008'; ctx.fillRect(x-5,GROUND_Y-32,10,48);
    ctx.fillStyle='rgba(35,110,35,0.8)'; ctx.beginPath(); ctx.arc(x,GROUND_Y-42,25,0,Math.PI*2); ctx.fill();
  });
}
function drawFire(){
  [55,180,305].forEach(x=>{
    const f=Math.sin(Date.now()*.007+x*.01)*7;
    const g=ctx.createRadialGradient(x,GROUND_Y,0,x,GROUND_Y+16,18+f);
    g.addColorStop(0,'#ff8800'); g.addColorStop(1,'transparent');
    ctx.fillStyle=g;
    ctx.beginPath(); ctx.ellipse(x,GROUND_Y-(13+f)*.4,(11+f)*.4,13+f,0,0,Math.PI*2); ctx.fill();
  });
}

// ── HUD — timer gros, tremblement <10s ───────────────────────
function drawHUD(){
  ...
  ctx.textAlign='left';
  ctx.fillText(`Niv.${currentLevel+1} ${LEVELS[currentLevel].name}`,8,26);
  ctx.textAlign='right';
  ctx.fillText(`🪙${coinsCollected} ⭐${starsCollected}/5`,W-8,26);

  // Timer — centré, gros
  const ss=String(timeLeft%60).padStart(2,'0');
  const timerStr=`${Math.floor(timeLeft/60)}:${ss}`;
  const panic = timeLeft<=10;
  const timerSize = panic ? 26 : 20;

  ctx.save();
  if(panic){
    // Tremblement
    const shake=(Math.random()-0.5)*6;
    ctx.translate(W/2+shake, 20+shake);
    ctx.fillStyle='#ff2222';
    // Halo rouge pulsant
    ctx.save();
    ctx.globalAlpha=0.3+Math.sin(Date.now()*0.02)*0.2;
    ctx.font=`bold ${timerSize+8}px Courier New`;
    ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText(timerStr,0,0);
    ctx.restore();
  } else {
  ...
  ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillStyle = panic ? '#ff2222' : (timeLeft<=15 ? '#ff8800' : '#ffd700');
  ctx.fillText(`⏱ ${timerStr}`,0,0);
  ctx.restore();
}
...
function showOverlay(){
  ...
  document.getElementById('overlay-coins').textContent=`🪙 Pièces : ${coinsCollected}`;
...
function showWin(){
  ...
  document.getElementById('overlay-coins').textContent=`🪙 Total : ${coinsCollected}`;
  const inp=document.getElementById('native-input');
